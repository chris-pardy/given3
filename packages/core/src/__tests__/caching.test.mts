...
import { createGivenConstructor } from "../index.mjs";
...
describe('caching', () => {
    ...
    beforeEach(() => {
        ...
        given = createGivenConstructor(hooks);
    })

    it.each(['All', 'Each'] as const)('given a cache of %s the value should only be computed once per test', async (cache) => {
    ...
    it.each(['Each', false] as const )('given a cache of %s the value should be computed for each test', async (cache) => {
        const definition = vi.fn(() => 1);
        await hooks.suite(async () => {
            const v = given('value', definition, { cache });
            await hooks.test(() => {
    ...
    describe('smart caching', () => {
        ...
        it('value checking reduces unnecessary calls', async () => {
            ...
            await hooks.suite(async () => {
                const dependency = given('dependency', dependencyDefinition, { cache: false });
