...
describe('the access control system', () => {
    ...
    const accessCheck = given('accesscheck', () => createAccessCheck(userService.value));
    const resource = given(() => 'viewUser');
    
    const userFactory = given(() => UserFactory.define(({ sequence }) => ({
    ...
    const accessCheckResult = given(() => accessCheck.value(user.value.id, resource.value));
    ...
    describe('given a known user', () => {
        ...
        it.each(['createUser', 'updateUser', 'deleteUser'])('should not allow access to %s', (resourceName) => {
        ...
        describe('given an admin user', () => {
            ...
            it.each(['createUser', 'updateUser', 'deleteUser', 'viewUser'])('should allow access to %s', (resourceName) => {
                resource.define(() => resourceName);
